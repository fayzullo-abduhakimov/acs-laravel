@php
    ...
    $seoPage = \Illuminate\Support\Facades\Cache::remember('seo.page.home', 3600, fn () => Page::where('name', 'home')->first());

    $seoTitle = $seoPage
        ? ($seoPage->getTranslation('meta_title', $locale) ?: $seoPage->getTranslation('title', $locale))
        : config('app.name');
...
<head>
    ...
    @foreach (LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
        <link rel="alternate" hreflang="{{ $localeCode }}" href="{{ LaravelLocalization::getLocalizedURL($localeCode, url()->current()) }}">
    @endforeach
